...
def _nearest_tracks(
    ...
    distances = cylinder_distances(candidates, target, box_length_x)[:, 0]
...
def _minimum_track_distance_near_death(
    ...
    for frame in range(start, stop):
        ...
        if (
            ...
            continue
        first = cylindrical_to_cartesian(table.positions[first_track_idx, offset][np.newaxis, :])
        second = cylindrical_to_cartesian(table.positions[second_track_idx, offset][np.newaxis, :])
        distance = float(cylinder_distances(first, second, box_length_x)[0, 0])
...
def _annihilation_pairs(
    ...
    *,
    ...
    for left_pos, left_idx in enumerate(death_track_indices):
        for right_idx in death_track_indices[left_pos + 1 :]:
