...
from .cases import HEXATIC_OUTPUT_DIR, NPZ_FIELDS_DIR, UnwrappedCase, ensure_output_dirs, get_case


def _trajectory_frame_count(case: UnwrappedCase) -> int:
    with gsd.hoomd.open(name=str(case.trajectory_gsd), mode="r") as source:
        return len(source)


def _shear_frame_indices(case: UnwrappedCase, stride: int) -> range:
    if stride <= 0:
