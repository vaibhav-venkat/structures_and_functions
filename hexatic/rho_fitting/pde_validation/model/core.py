...
def run_validation(inputs: ValidationInputs, options: ValidationOptions | None = None) -> ValidationResult:
    ...
    payload = _rho_fitting_core.run_pde_validation(
        np.ascontiguousarray(inputs.rho, dtype=np.float64),
        np.ascontiguousarray(inputs.p, dtype=np.float64),
        np.ascontiguousarray(inputs.q, dtype=np.float64),
        np.ascontiguousarray(inputs.psi6_sq, dtype=np.float64),
        np.ascontiguousarray(inputs.y_p, dtype=np.float64),
        np.ascontiguousarray(inputs.times, dtype=np.float64),
