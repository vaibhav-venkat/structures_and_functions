...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run all big-Lx simulations, then analyze them after a barrier."
    )
    parser.add_argument("--all", action="store_true")
    parser.add_argument("--case", action="append", default=[])
    parser.add_argument("--output-root", type=Path, default=DEFAULT_OUTPUT_ROOT)
    parser.add_argument("--gpu-ids", default=None)
    parser.add_argument("--simulation-workers", type=int, default=5)
    parser.add_argument("--analysis-workers", type=int, default=3)
    parser.add_argument(
        "--simulation-device", choices=("gpu", "cpu"), default="gpu"
    )
...
def main() -> None:
    ...
    print("[big_lx.pipeline] simulation barrier complete; stage=analysis", flush=True)
