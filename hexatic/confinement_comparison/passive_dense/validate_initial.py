...
from .geometry import generate_dense_2d, generate_passive_cylinder
...
def _validate_dense_2d(case) -> dict[str, object]:
    ...
    if not np.allclose(np.mean(positions, axis=0), 0.0, atol=1e-12):
        raise AssertionError("dense 2D lattice is not centered")
    ...
    first = positions[: case.ny]
    last = positions[-case.ny :]
