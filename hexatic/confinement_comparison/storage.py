...
def save_safetensors_atomic(
    ...
    *,
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    temporary = path.with_suffix(path.suffix + ".tmp")
    contiguous = {name: np.ascontiguousarray(value) for name, value in tensors.items()}
    if backend_name == "jax":
...
def prepare_analysis_dir(
    ...
    if manifest_path.exists() and not overwrite:
        ...
        raise FileExistsError(
            f"Incomplete analysis exists at {output_dir}; use --resume or --overwrite"
        )
...
class FrameShardWriter:
    ...
    def flush(self) -> None:
        ...
        save_safetensors_atomic(
            ...
            metadata={
                "schema": "hexatic.confinement_comparison.frames.v1",
