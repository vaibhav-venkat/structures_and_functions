...
pub(crate) struct TemporalOperators {
    ...
    pub(crate) scaled_times: Array1<f64>,
...
pub(crate) struct TemporalFieldResult {
    pub(crate) cleaned: ArrayD<f64>,
    pub(crate) filtered: ArrayD<f64>,
    pub(crate) derivative: ArrayD<f64>,
    pub(crate) coefficients: ArrayD<f64>,
}

impl TemporalOperators {
    pub(crate) fn new(
        steps: ArrayView1<'_, i64>,
        timestep: f64,
        cutoff: usize,
    ) -> CoreResult<Self> {
        if steps.is_empty() {
    ...
    pub(crate) fn apply(&self, values: ArrayViewD<'_, f64>) -> CoreResult<TemporalFieldResult> {
    ...
    pub(crate) fn diagnostic_coefficients(
        &self,
